...
int main(int argc, char* argv[]) {
    ...
    int64_t rotNum=slot_count/lut_size;
    ...
    if (Serial::DeserializeFromFile(table_in, ct_lut_x, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    if (Serial::DeserializeFromFile(table_out, ct_lut_y, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    if (Serial::DeserializeFromFile(table_min, enc_Lutxmin, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    if (Serial::DeserializeFromFile(table_max, enc_Lutxmax, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    vector<vector<double>> preY(ct_lut_y.size(),vector<double>(slot_count,0));
    vector<Ciphertext<DCRTPoly>> ct_pre_lut_y(preY.size());
    for (size_t i=0;i<ct_lut_y.size();i++){
        preY[i][rotNum*i]=1;
    }
    for(size_t i=0;i<preY.size();i++){
        Plaintext encoded_pre_lut_y    = context->MakeCKKSPackedPlaintext(preY[i]);
        ct_pre_lut_y[i]=context->Encrypt(public_key, encoded_pre_lut_y);
    }
    ...
    for(int testCount=0;testCount<test_num;testCount++){
        ...
        Ciphertext<DCRTPoly> encrypted_result;
        encrypted_result = twoInput_Li(ct_query,ct_lut_x, ct_lut_y,ct_pre_lut_y,context,n,d,lut_size);
        ...
        double expect_output=query[0]+query[1];
        outputs.push_back(result);
        expect_outputs.push_back(expect_output);

        std::cout << "input: " ;
        for(auto ele:query) std::cout << ele << ",";
        std::cout << endl;
