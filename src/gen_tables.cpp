...
int main(int argc, char *argv[]) {
  ...
  // if
  // (Serial::DeserializeFromFile("data/Key/"+type+"/"+to_string(bitNum)+"bit/SecretKey",
  ...
  if (type == "oneInput") {
    auto lut_pair =
        generateLUT_oneInput_propsed(bitNum, Lutxmax, Lutxmin, context);
    lut_x = lut_pair.first;
    lut_y = lut_pair.second;
  } else if (type == "twoInput") {
    auto lut_pair =
        generateLUT_twoInput_propsed(bitNum, Lutxmax, Lutxmin, context);
    lut_x = lut_pair.first;
    lut_y = lut_pair.second;
  } else if (type == "threeInput") {
    auto lut_pair =
        generateLUT_threeInput_propsed(bitNum, Lutxmax, Lutxmin, context);
    lut_x = lut_pair.first;
    lut_y = lut_pair.second;
  ...
  for (int i = 0; i < slot_count; i++)
    vec_Lutxmax.push_back(1.0 / (Lutxmax - Lutxmin));
  ...
  if (!Serial::SerializeToFile(table_in, ct_lut_x, SerType::BINARY)) {
    std::cerr << "Error writing serialization of ciphertext 1 to " << table_in
              << std::endl;
    return 1;
  }

  if (!Serial::SerializeToFile(table_out, ct_lut_y, SerType::BINARY)) {
    std::cerr << "Error writing serialization of ciphertext 1 to " << table_out
              << std::endl;
    return 1;
  }

  if (!Serial::SerializeToFile(table_min, enc_Lutxmin, SerType::BINARY)) {
    std::cerr << "Error writing serialization of ciphertext 1 to " << table_min
              << std::endl;
    return 1;
  }
  if (!Serial::SerializeToFile(table_max, enc_Lutxmax, SerType::BINARY)) {
    std::cerr << "Error writing serialization of ciphertext 1 to " << table_max
              << std::endl;
    return 1;
  }
