...
pair<vector<vector<double>>, vector<vector<double>>> generateLUT_oneInput_propsed(int bit_num, 
    double Lutxmax,
    double Lutxmin,
    ...
    // interval ensuring lutSize samples span [Lutxmin, Lutxmax] with both endpoints included
    double interval=(Lutxmax-Lutxmin)/(lutSize-3);
    cout<<"interval:" << interval <<endl;
    for(double i = Lutxmin ; i<Lutxmax; i+=interval){
      if(int(initInput.size())==lutSize-3) break;
    ...
    initInput[initInput.size()-1]=(initInput[initInput.size()-1]-Lutxmin)/(Lutxmax-Lutxmin);
    //step4: append value
    ...
    vector<vector<double>> lut_x(1,vector<double>(slot_count,0.0)),lut_y(1,vector<double>(slot_count,0.0));
...
pair<vector<vector<double>>, vector<vector<double>>> generateLUT_twoInput(int bit_num, 
                          double Lutxmax,
                          double Lutxmin,
                          CryptoContext<DCRTPoly>& context){
    int inputNum=2;
...
pair<vector<vector<double>>, vector<vector<double>>> generateLUT_twoInput_propsed(int bit_num, 
                          double Lutxmax,
                          double Lutxmin,
                          CryptoContext<DCRTPoly>& context){
    int inputNum=2;
    ...
    vector<vector<double>> rearrangedOutput(pow(lutSize,inputNum-1),vector<double>(lutSize,0.0));
    for(int k=0;k<pow(lutSize,inputNum-1);k++){
        for(int j=0;j<lutSize;j++){
            rearrangedOutput[k][j]=initOutput[ ( (k / static_cast<size_t>(std::pow(lutSize, inputNum - 2)))
...
pair<vector<vector<double>>, vector<vector<double>>> generateLUT_threeInput_propsed(int bit_num, 
        double Lutxmax,
        double Lutxmin,
        CryptoContext<DCRTPoly>& context){
    ...
    // Generate output
    //step2: create output table
    vector<vector<vector<double>>> initOutput(lutSize,vector<vector<double>>(lutSize,vector<double>(lutSize, 0.0)));
    for(double i=0;i<initInput.size();i++){
        for(double j=0;j<initInput.size();j++){
        ...
        initOutput[i][j][initInput.size()]=initOutput[i][j][initInput.size()-1];
        }
        //step4: append value
        initOutput[i][initInput.size()]=initOutput[i][initInput.size()-1];
    }
    ...
    for(int k=0;k<pow(lutSize,inputNum-1);k++){
        for(int j=0;j<lutSize;j++){
            rearrangedOutput[k][j]=initOutput[ ( (k / static_cast<size_t>(std::pow(lutSize, inputNum - 2)))
                % static_cast<size_t>(std::pow(lutSize, inputNum - 1))
                + j ) % lutSize][ ( (k / static_cast<size_t>(std::pow(lutSize, inputNum - 3)))
