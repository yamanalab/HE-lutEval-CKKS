...
void func(Ciphertext<DCRTPoly>& x,
  ...
  if(n>1) x5=context->EvalMult(x5,func_coeff[5]); 
  if(n>2) x7=context->EvalMult(x7,func_coeff[7]); 
  if(n>3) x9=context->EvalMult(x9,func_coeff[9]); 
...
Ciphertext<DCRTPoly> PartialSum(Ciphertext<DCRTPoly> ctxt, int64_t slot_count, int64_t LUTnum, const CryptoContext<DCRTPoly> &cryptoContext){
...
Ciphertext<DCRTPoly> oneInput_proposed(Ciphertext<DCRTPoly> query,
  ...
  for(int i=0;i<row_size;i++){
    ...
    Ciphertext<DCRTPoly> result_row=PartialSum(comp_y, slot_count,lut_size, context);
...
Ciphertext<DCRTPoly> twoInput_Li(vector<Ciphertext<DCRTPoly>> query,
    ...
    auto Comp_result_begin_time = high_resolution_clock::now();
    int rotNum=slot_count/lut_size;
    for(int i=0;i<2;i++){
    ...
    auto Comp_result_end_time = high_resolution_clock::now();
    duration<double> Comp_result_sec =
        Comp_result_end_time - Comp_result_begin_time;
    ...
    Ciphertext<DCRTPoly> result=PartialSum(lut_cty[0], slot_count,lut_size, context);
    return result;
}
...
Ciphertext<DCRTPoly> twoInput_propsed(vector<Ciphertext<DCRTPoly>> query,
  ...
  //step4
  Ciphertext<DCRTPoly> result=PartialSum(oneHot_tbl[0], slot_count,lut_size, context);
