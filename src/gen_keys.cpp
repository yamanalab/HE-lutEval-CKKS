...
int main(int argc, char *argv[]) {
  ...
  if (!Serial::SerializeToFile("data/Key/" + type + "/" + to_string(bitNum) +
                                   "bit/Params",
  ...
  if (!Serial::SerializeToFile("data/Key/" + type + "/" + to_string(bitNum) +
                                   "bit/PublicKey",
                               key_pair.publicKey, SerType::BINARY)) {
    std::cerr << "Error writing serialization of public key to util/Key/" +
                     type + "/" + to_string(bitNum) + "bit/PublicKey.txt"
              << std::endl;
    std::exit(1);
  }
  ...
  if (!Serial::SerializeToFile("data/Key/" + type + "/" + to_string(bitNum) +
                                   "bit/SecretKey",
                               key_pair.secretKey, SerType::BINARY)) {
    std::cerr << "Error writing serialization of private key to "
                 "util/Key/SecretKey.txt"
              << std::endl;
    return 1;
  }

  std::ofstream emkeyfile("data/Key/" + type + "/" + to_string(bitNum) +
                              "bit/RelinKey",
  ...
  std::ofstream erkeyfile("data/Key/" + type + "/" + to_string(bitNum) +
                              "bit/RotateKey",
                          std::ios::out | std::ios::binary);
  if (erkeyfile.is_open()) {
    if (context->SerializeEvalAutomorphismKey(erkeyfile, SerType::BINARY) ==
        false) {
