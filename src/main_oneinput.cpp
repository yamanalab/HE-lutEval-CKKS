...
int main(int argc, char *argv[]){
    ...
    if (Serial::DeserializeFromFile(table_in, temp_enc_in, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    if (Serial::DeserializeFromFile(table_out, temp_enc_out, SerType::BINARY) == false) {
      std::cerr << "Could not read the ciphertext" << std::endl;
      return 1;
    }
    ...
    for(int testCount=0;testCount<test_num;testCount++){
      ...
      for(int i=0;i<slot_count;i++) input_vec.push_back(random[testCount]);
      ...
      Ciphertext<DCRTPoly> result= oneInput_proposed(temp_input,temp_enc_in,temp_enc_out,context,n,d,pow(2,bitNum));
  ...
  cout<<"avelatency:"<<accumulate(latencyList.begin(), latencyList.end(), 0.0) / double(latencyList.size()) <<endl;
